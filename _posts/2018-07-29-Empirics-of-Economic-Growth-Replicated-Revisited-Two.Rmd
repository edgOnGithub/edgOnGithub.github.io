...
Ideally when generating such a large number of draws we'd use some form of parallel processing as this is an example of an embarassingly parralel problem.^[https://en.wikipedia.org/wiki/Embarrassingly_parallel.] Fortunately, Davis Vaughan has recently developed furrr, a parallel implementation of `map` using the R's future library. ~~Unfortunately, I ran into an issue with dplyr's quasiquotation and `future_map` so the current code uses the traditional `map`.~~^[~~I've opened an issue here:~~ Davis has replied with a simple workaround! https://github.com/DavisVaughan/furrr/issues/25.]
