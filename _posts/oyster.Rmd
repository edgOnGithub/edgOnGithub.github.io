...
Here we just use an example split to train the model:
...
future_df <- make_future_dataframe(model, freq = 60,
                                   periods = 60*24)
forecast <- predict(model, future_df) %>% as_tibble() %>% 
  select(ds, yhat)

train_and_prediction <- left_join(forecast, prophet_train, by = "ds")
...
df_cv <- cross_validation(model, horizon = 60, units = "mins")
df_cv %>% head %>% kable
```
