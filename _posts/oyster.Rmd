...
In this post we're departing a little from replication of papers in economics and trying our hand at visualising networks followed by modelling passenger footfall across the London Underground during a week in November 2009. 
```{r, include = FALSE}
library(readr)
library(dplyr)
library(knitr)
...
## Cleaning Data
The dataset we use comes from TFL's open data [portal](https://tfl.gov.uk/info-for/open-data-users/) and represents a random sample (I hope) of TFL's Oyster Card data during on week in November - the data provided represents approximately 5\% of all journey's during this period.
...
Next we encode `daytype` as a factor with levels running from Monday to Sunday, we also add a weekday and weekend factor that will make histograms featuring aggregated data later easier to order. Finally, we convert the timestamp TFL use to a more accessible format using lubridate's `ymd_hms()` and create `journey_time` using lubridate's `interval()` function.
...
Now we bin each time period into five minute intervals and plot mean journey time as a function of tap in time. Here we bin the intervals using two dplyr pipes and lubridate, alternatively we could have used the tibbletime library which we'll explore briefly later.
...
LOESS lines show a little clearer than the scatter plot that it seems like weekends have higher average journey times. This is a little counter-intuitive as at first glance we'd expect weekday journeys to be longer due to rush hour traffic. However, a simple explanation (classic confounder relationship) is that commuter preferences change when comparing weekdays and weekends - short commutes to work will have different mean journey times than sightseeing trips on the weekend for instance.

To formally test for this^[Really and truly we should make adjustments to our standard errors here for heteroscedasticity and autocorrelation]:
```{r, results = "asis"}
library(broom)
...
results <- model %>% tidy()
confints <- model %>% confint_tidy()
estimates_intervals <- bind_cols(results, confints)

estimates_intervals %>% xtable()
```


## Network Visuallisations

Now we move onto visuallising the network aspect of London's Tube data. Here we `group_by` starting and closing station and find the number of journeys, mean journey length and mean fare.
...
First however, some more visulations:
...
In this section we're going to try and predict weekday footfall using an LTSM neural net - we'll draw heavily on Rstudio's Keras blog post [here](https://blogs.rstudio.com/tensorflow/posts/2018-06-25-sunspots-lstm/). In this post we're going to fit the neural net with minimal tweaks or tuning and visually inspect model results rather than formally comparing with alternative methods.

I think there are two valid approaches from an OLS point of view in this case. We can either ignore the temporal element and bin observations up into time periods with various dummies and interactions for station closures/rush hour or we can use some sort of AR/MA model.

The motivation for our LTSM approach is that I'm curious how well an out of the box, fancy neural net will compare with traditional econometric methods.
...
Here we just use the final example split to train the model:
...
The [recipes](https://tidymodels.github.io/recipes/) library is very useful for scaling and standardising data:
```{r}
rec_obj <- recipe(y ~ ., df) %>% 
  step_sqrt(y) %>% 
  step_center(y) %>% 
...
This code block transforms the data into "tensors" as required by Keras/TensorFlow: 
