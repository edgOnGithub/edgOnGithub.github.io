...
oyster_underground_monday <- oyster_underground_monday %>% 
  mutate(journey_time = difftime(EXTimeHHMM, EntTimeHHMM, units = "mins")) %>% 
  filter(journey_time > 0)

oyster_j_time <- oyster_underground_monday %>% 
  group_by(EntTimeHHMM) %>% 
  summarise(average_journey = mean(journey_time))
  
journey_time_plot <- plot_ly(oyster_j_time,
                             x = ~EntTimeHHMM %>% 
                               as.POSIXct(),
                             y = ~average_journey,
                             type = "scatter",
