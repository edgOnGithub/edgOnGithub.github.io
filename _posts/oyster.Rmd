...
Now we create loss plots and a comparison dataframe:
...
Googling suggests that if training loss is above validation loss it indicates that the training set is much harder to predict than our validation set - I think that the footfall 0s during closing hours are causing this rather than any coding/modelling errors on my part but I could be very wrong.
...
Here I depart from the RStudio blog post and transform the data into a long format which makes plotting significantly easier. I'm quite lazy here and use `separate` rather than `gsub` and regex to remove unwanted strings.
...
Here we do the same but on the test set.
