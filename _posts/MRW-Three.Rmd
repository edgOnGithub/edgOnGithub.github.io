...
MRW_clean <- readr::read_csv('assets/Transformed Data/MRW_appendix_clean.csv') %>% 
  mutate(n_g_d = (working_age_pop_ch + 5)/ 100,
         s = s / 100)
solow_MRW <- readr::read_csv('assets/Transformed Data/solow_MRW.csv')
...
It makes sense to me to replace MRW's original dummy structure with these new, mutually exclusive dummies. Instinctively, using non-mutually exclusive dummies to partition the dataset with which to run the regressions seems wrong to me - we're oversampling some datapoints in my opinion - I discuss this more in the post's conclusion.
...
generate.tsne <- function(perplexity, data){
  data <- data %>% 
    na.omit 
...
I think this comes down to the question: What population parameter are the authors trying to uncover? If the aim is to identify the parameter for each group e.g. for policy reasons or because they're of interest in their own right I don't see a problem. However, if the aim is to empirically identify the Solow model and these subsets are just drawn from some population, global hyperparameter I think MRW's approach is questionable if not incorrect. Finally, regardless of the above, why not include the dummies and their interactions in a pooled model and at the very least include our nine excluded observations? 

Regardless, econometrics has moved lightyears since this paper and hindsight is 20/20
