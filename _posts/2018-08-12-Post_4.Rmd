...
```{r}
library(knitr)
estimated_parameters_new <- estimated_parameters_new %>% 
  filter(subset != 'Oil')


comparison_tbl <- bind_rows(estimated_parameters_new %>%
...
```{r, modelling}
k <- rep(seq(from = 0, to = 25, by = .5), nrow(MRW_clean))
countries <- rep(MRW_clean$country, 51)

country_tbl <- tibble('k' = k,
                      'country' = countries) %>% 
  left_join(y = MRW_clean %>% 
              select(s, n_g_d, country), by = 'country') %>% 
  mutate(saving_curve_)
...
savings <- k %>% 
  map_df(find.saving.point,
         alpha = alpha_uk,
         s = s_uk)
eff_dep <- k %>% 
  map_df(find.effective.depreciation,
         n = n_g_d_uk,
         g = 0,
         d = 0)

plot_tbl <- bind_cols(eff_dep, savings)
plot_tbl$k <- k
